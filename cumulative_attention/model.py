...
class FashionSentenceGenerator(nn.Module):
    ...
    def prepare_memory(self, batch_data):
        ...
        self.key_memory = torch.zeros(self.batch_size, self.max_mem_size, self.embedding_dim, dtype=torch.float, device=device)
        self.value_memory = torch.zeros(self.batch_size, self.max_mem_size, self.embedding_dim, dtype=torch.float, device=device)
    ...
    def apply_attention_HN(self, prev_hiddens):
        ...
        concatenated = self.attn_HN(torch.cat((self.hist_N.view(self.batch_size,-1), prev_hiddens), 1).view(self.batch_size, 1, -1))
